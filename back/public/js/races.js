...
const dndRaceToHTML = (race, index) => {
    ...
    <div class="${carousel_class}">
        <div class="card text-white bg-dark text-center">
            ...
            <div class="card-body">
                ...
                <p class="card-text">${race.smallDescription}</p>
                <a href="./classDetail.html" class="btn btn-primary">View Detail</a>
                <p class="back-img" hidden>${race.background}</p>
            </div>
        </div>
    </div>`
}

const dndRaceListToHTML = (list, id) => {
    if (id && list && document.getElementById(id)) {
        document.getElementById(id).innerHTML = list.map(dndRaceToHTML).join('\n');
    }
}

function getRaces() {
    let url = APIURL + "/races";
    sendHTTPRequest(url, null, HTTTPMethods.get, (res) => {
        let data = JSON.parse(res.data)
        dndRaceListToHTML(data['content'], 'carousel');
    }, (res) => {
    }, getTokenValue('token'));
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $('#filterRace').on('change', function (event) {
        let names = [];
        let order = [];
        ...
        let find = names.findIndex(el => el.toLowerCase().startsWith( $('#filterRace').val().toLowerCase() ));
